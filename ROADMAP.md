...
**Status:** In progress on the `kind-aware-numbers` branch. Phase 1 (relations + algebra rewiring) merged in PR #256; Phase 2 (`Bridge` + `UnitSystem.adopt`) merged in PR #257; Phase 3 introduces the `ActiveContext` substrate so the `_active` ContextVar carries `system + formulas + kinds + strict` as a single payload, paired with typed accessors and an extended `use(...)` that inherits unset fields. Remaining v2.0 work: `Number.kind` field + `bound_to` predicate, arithmetic dispatch through `active_formulas()`, strict source-unit resolution, decorator/pydantic/TOML kind extensions.
