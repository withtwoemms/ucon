...
| v1.9.1 | **Aspect propagation.** Activates the `aspect_rules` field on `KindFormula`. Adds `ucon.aspects` subpackage with `AspectSet`, `AspectJoinPolicy`, and `join_aspects(a, b, policy)`. Adds `KindFormula.project_aspects` and `FormulaRegistry.apply(inputs)` returning `(formula, output_kind, output_aspects)`. Still callable-side; still not wired to `Number`. | Pending release |
