...
| v1.9.1 | **Aspect propagation.** Activates the `aspect_rules` field on `KindFormula`. Adds `AspectSet` (`frozenset[str]`), `FormulaRegistry.apply(inputs)` returning `(formula, output_kind, output_aspects)`, and `join_aspects(a, b, policy)` for combining aspect sets under LCA. Still callable-side; still not wired to `Number`. | Proposed |
| v1.9.2 | **Lookup completeness.** Makes the registry consult declared structure that v1.9.0 indexes but does not act on: full `n!` permutation for commutative formulas with `n > 2`, ancestor-walk matching for formulas declared with `generalizes=True`, and opt-in dimension-only fallback. Adds `LookupResult` with `MatchKind` (EXACT/GENERALIZED/DIMENSIONAL) for diagnostic inspection. No new concepts — completeness pass on existing declarations. | Proposed |
...
**Status:** v1.9.0 shipping. v1.9.1 and v1.9.2 designed but not yet branched. Full integration in v2.0.
