...
| **v2.0** | **`UnitSystem` as a value type** — promote `UnitSystem` to a first-class container of units, dimensions, conversions, basis, and basis-graph; rename today's class to `BaseUnits`; retire module-global registries; make `Vector` strictly same-basis with explicit cross-basis helpers (subsumes the basis subpackage cycle break) | Proposed |
