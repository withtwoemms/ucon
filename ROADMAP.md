...
| v2.1.x | **Correctness train** — six per-issue patch releases (2.1.2–2.1.7) closing the 2026-09 audit: exact customary prefactors derived from statutory seeds, constant symbols in package edge factors, path resolution through composite edge endpoints, typed `DisjointKinds` refusals, idempotent kind re-add, exact SI `base_form`s for mechanical CGS units. Per-issue detail in `CHANGELOG.md`; delivery record in `docs/internal/RELEASE_PLAN_v212-v310.md`. | Complete |
| v2.2.0 | **Package infrastructure** — `[[contexts]]` in TOML, `PolynomialMap` with branch inverses, declared `Unit.chart` (`docs/internal/decisions/010-chart-nomenclature.md`), `default_kind`, first deprecation rung (pseudo-dimensions, `AspectSet`). | Planned |
| v2.3.0 | **Aspect stratum** — per `docs/internal/decisions/008-aspect-stratum.md`: one `Aspect` type (peer of `Kind`), flat additive `Number.aspects`, family-wise resolution, carry rule, D3 namespacing. | Planned |
...
| v3.1.0 | **The Turnstile** — per `docs/internal/decisions/009-turnstile.md`: four-stratum arithmetic dispatch (dimension, chart, kind, aspect), derived point/displacement, cyclic/ordinal kind flags, lazy warrants via `explain()`. New refusals on previously-wrong paths are fixes. | Planned |
...
**Status:** Complete. All three releases shipped; full integration landed in v2.0.0.
...
**Status:** Complete. Released as v2.0.0 on 2026-06-07.
