...
def parse_dimension(spec: str, basis: Basis | None = None) -> "Dimension":
    """Parse a dimension string into a :class:`Dimension`.
...
class _DimensionParser:
    ...
    Reuses :class:`ucon.parsing._Tokenizer` to share lexer behaviour with
    the unit parser (operators, parentheses, ASCII ``^N`` exponents,
    ...
    def __init__(self, spec: str, basis: Basis) -> None:
        # Defer import to avoid a load-time cycle (parsing imports from core,
        # which transitively touches dimension during registry warm-up).
        from ucon.parsing import _Tokenizer, _TokenType
    ...
    def parse(self) -> "Dimension":
        ...
        if self._current.type != self._TT.EOF:
            raise ValueError(
    ...
    def _parse_expr(self) -> "Dimension":
        ...
        while self._current.type in (self._TT.MUL, self._TT.DIV):
            op = self._advance()
            right = self._parse_term()
            if op.type == self._TT.MUL:
                left = left * right
            else:
                left = left / right
        return left
    ...
    def _parse_term(self) -> "Dimension":
        ...
        if self._current.type == self._TT.POW:
            self._advance()
            exp_tok = self._advance()
            if exp_tok.type != self._TT.NUMBER:
                raise ValueError(
        ...
        if self._current.type == self._TT.NUMBER:
            exp_tok = self._advance()
            return base ** _to_exponent(exp_tok.value)
        return base

    def _parse_factor(self) -> "Dimension":
        if self._current.type == self._TT.LPAREN:
            self._advance()
            expr = self._parse_expr()
            if self._current.type != self._TT.RPAREN:
                raise ValueError(
        ...
        if self._current.type == self._TT.NUMBER and self._current.value == "1":
            self._advance()
            return NONE
        if self._current.type == self._TT.IDENT:
            tok = self._advance()
