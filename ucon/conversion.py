...
Implements the **Graph** (aliased as ``ConversionGraph`` for backward
compatibility) — the registry of unit conversion morphisms that enables
`Number.to()` conversions.

Classes
-------
- :class:`Graph` — Stores and composes conversion Maps between units.
...
class Graph:
    ...
    def add_edge(
        ...
        *,
        ...
        :class:`Graph` with domain-specific units. Call it on
        either the default package graph (``ucon.graph.default_graph``) or
        your own ``Graph`` instance to wire new units into the
        conversion system.
        ...
        See Also
        --------
        Graph.convert : Path-find between registered units.
        Graph.connect_systems : Bulk-add cross-basis edges.
    ...
    def with_package(self, package: 'UnitPackage') -> 'Graph':
    ...
    def _package_edge_already_covered(
        ...
        with using_conversion_graph(graph):
            try:
                src_unit = _parse_unit(edge_def.src)
                dst_unit = _parse_unit(edge_def.dst)
...
@contextmanager
def using_conversion_graph(graph: Graph):
    """Context manager for scoped :class:`Graph` override.
