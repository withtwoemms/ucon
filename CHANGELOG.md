...
- **The publish workflow refuses to release a tag whose changelog section
  is missing.** (#311) Release notes are generated by scanning
  `CHANGELOG.md` for `## [<tag>]` at tag-push time. When the content was
  still under `## [Unreleased]`, the scan matched nothing, wrote an empty
  file, and the GitHub release was created with a blank body — silently,
  with every step reporting success. Seven releases shipped that way:
  `2.1.2` through `2.1.7`, plus `2.1.2a1`.

  A preflight now runs before the build and before any upload, so a
  missing section fails the run while it is still cheap to fix — promote
  the section, move the tag, push again. Guarding after the upload would
  instead leave the package public and the run red, with a re-run
  skipping PyPI and failing the same check. Alpha and beta tags are
  exempt, matching the mainline-ancestry check's existing exemption,
  since those may come from feature branches. A second check rejects a
  section header that matches but has no body.
